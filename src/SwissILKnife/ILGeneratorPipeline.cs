...
namespace SwissILKnife
{
	public class ILGeneratorPipeline : AbstractILGenerator.AbstractILGenerator
	{
		...
		public override Label BeginExceptionBlock()
			=> Cache(_all.All((ilgen) => ilgen.BeginExceptionBlock()));

		public override LocalBuilder DeclareLocal(Type localType)
			=> Cache(_all.All((ilgen) => ilgen.DeclareLocal(localType)));

		public override LocalBuilder DeclareLocal(Type localType, bool pinned)
			=> Cache(_all.All((ilgen) => ilgen.DeclareLocal(localType, pinned)));

		public override Label DefineLabel()
			=> Cache(_all.All((ilgen) => ilgen.DefineLabel()));

		public override int ILOffset => _main.ILOffset;

		public override void BeginCatchBlock(Type exceptionType)
			=> _all.All((ilgen) => ilgen.BeginCatchBlock(exceptionType));

		public override void BeginExceptFilterBlock()
			=> _all.All((ilgen) => ilgen.BeginExceptionBlock());

		public override void BeginFaultBlock()
			=> _all.All((ilgen) => ilgen.BeginFaultBlock());

		public override void BeginFinallyBlock()
			=> _all.All((ilgen) => ilgen.BeginFinallyBlock());

		public override void BeginScope()
			=> _all.All((ilgen) => ilgen.BeginScope());

		public override void Emit(OpCode opcode)
			=> _all.All((ilgen) => ilgen.Emit(opcode));

		public override void Emit(OpCode opcode, byte arg)
			=> _all.All((ilgen) => ilgen.Emit(opcode, arg));
		...
		public override void Emit(OpCode opcode, Label label)
			=> _all.All(FindLabels(label), (ilgen, i, emitlabel) => ilgen.Emit(opcode, emitlabel));
		...
		public override void Emit(OpCode opcode, Label[] labels)
		{
			...
			_all.All((ilgen, i) => ilgen.Emit(opcode, forEveryLabel[i]));
		}

		public override void Emit(OpCode opcode, LocalBuilder local)
			=> _all.All(FindLocals(local), (ilgen, i, emitlocal) => ilgen.Emit(opcode, emitlocal));
		...
		public override void Emit(OpCode opcode, SignatureHelper signature)
			=> _all.All((ilgen) => ilgen.Emit(opcode, signature));
		...
		public override void EmitCall(OpCode opcode, MethodInfo methodInfo, Type[] optionalParameterTypes)
			=> _all.All((ilgen) => ilgen.EmitCall(opcode, methodInfo, optionalParameterTypes));
		...
		public override void EmitCalli(OpCode opcode, CallingConventions callingConvention, Type returnType, Type[] parameterTypes, Type[] optionalParameterTypes)
			=> _all.All((ilgen) => ilgen.EmitCalli(opcode, callingConvention, returnType, parameterTypes, optionalParameterTypes));

		public override void EmitWriteLine(FieldInfo fld)
			=> _all.All((ilgen) => ilgen.EmitWriteLine(fld));

		public override void EmitWriteLine(LocalBuilder localBuilder)
			=> _all.All((ilgen) => ilgen.EmitWriteLine(localBuilder));

		public override void EmitWriteLine(string value)
			=> _all.All((ilgen) => ilgen.EmitWriteLine(value));

		public override void EndExceptionBlock()
			=> _all.All((ilgen) => ilgen.EndExceptionBlock());

		public override void EndScope()
			=> _all.All((ilgen) => ilgen.EndScope());

		public override void MarkLabel(Label loc)
			=> _all.All(FindLabels(loc), (ilgen, i, emitlabel) => ilgen.MarkLabel(emitlabel));

		public override void ThrowException(Type excType)
			=> _all.All((ilgen) => ilgen.ThrowException(excType));

		public override void UsingNamespace(string usingNamespace)
			=> _all.All((ilgen) => ilgen.UsingNamespace(usingNamespace));
	}

	internal static class Extension
	{
		public static void All(this ILGenerator[] ilgenerators, Action<ILGenerator> ilgenAction) => ilgenerators.All((ilgen, i) => ilgenAction(ilgen));

		public static void All(this ILGenerator[] ilgenerators, Action<ILGenerator, int> ilgenAction)
		{
		...
		public static void All<T>(this ILGenerator[] ilgenerators, T[] items, Action<ILGenerator, int, T> ilgenAction)
		{
			for (int i = 0; i < ilgenerators.Length; i++)
			{
